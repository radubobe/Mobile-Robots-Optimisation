...
from Init import *
from road_oracle import (RoadOracle, load_spine, resolve_road_name,
                         resolve_road_width)
oracle = RoadOracle(load_spine(resolve_road_name()),
                    half_width=resolve_road_width() / 2.0,
...
#setarea weighturilor in P sistem 
    ...
    if oracle.update(gps_values[0], gps_values[1], lw, rw) != RoadOracle.RUNNING:
        print(oracle.report())
        oracle.write_report("verdict_%s.json" % time.strftime("%d-%m-%Y_%H-%M-%S"))
        break
    #print(value)
